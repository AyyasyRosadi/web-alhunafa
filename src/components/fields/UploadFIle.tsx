...
function UploadFile({ title,video, preview, type, setValue, value, placeholder, remove }: UploadFileAttributes) {
        ...
        {preview ? (
          ...
          <div className="flex items-center space-x-2 h-[15rem] w-full hover:bg-slate-200 transition-colors ease-in-out duration-300">
            <div className="flex md:flex-row flex-col space-x-2 w-full justify-center">
              <h1 className="text-xl text-center font-bahij">{title}</h1>
              <h1 className="text-xl font-thin">{type}</h1>
            </div>
          </div>
        )}
        <input ref={inputRef} name="input" type='file' value={value} onChange={setValue} className={`hidden`} placeholder={'ملف الإدخال'} />
      </div>
