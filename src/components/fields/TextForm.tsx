...
function InputForm({ id, max, step, type, title, className, read, icon, method, methodName, whiteText, noMessage,right }: InputFormAttributes) {
    ...
    return (
        <div className={`flex flex-col relative w-[100%]`}>
            ...
            <div className='flex w-full'>
                <input id={id} {...method.register(methodName)} max={max} step={step} type={type} className={`py-[7px] px-2 outline-none border border-gray-400 mt-1 text-gray-800 hover:border-gray-400 w-full font-montserrat ${className}  ${right ? 'text-right' : ''}`} readOnly={read} />
                {icon}
            </div>
