...
export default function Card({ src, title, location, children }: { src: StaticImport | string, title: string, location?: string, children?: ReactNode }) {
    ...
    return (
        <div onClick={() => location && replace(location)} className="relative flex flex-col px-4 py-6 text-gray-700 bg-white shadow-xl bg-clip-border rounded-xl xl:w-[32%] md:w-[45%] h-[35rem] hover:scale-105 transition-all ease-in-out duration-300 cursor-pointer overflow-y-auto scrollbar-hide">
