...
export default function Card({ src, title, location, children }: { src: StaticImport | string, title: string, location?: string, children?: ReactNode }) {
    ...
    return (
        <div onClick={() => location && push(location)} className="relative flex flex-col px-4 py-6 text-gray-700 bg-white shadow-xl bg-clip-border rounded-xl xl:w-[32%] md:w-[45%] h-[100%] hover:scale-105 transition-all ease-in-out duration-300 cursor-pointer">
            <Image placeholder='blur' loading='lazy' blurDataURL='../../assets/images/example.jpg' src={src} width={1000} height={1000} alt="" className="relative object-bottom object-cover rounded-xl w-[100%] h-[90%]" />
